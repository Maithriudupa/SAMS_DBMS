...
export default function SemTable(props) {
  ...
  const [sem,setSem] = React.useState('');
    const getPosts = async () => {
      try {
    const userPosts = await Axios.get("http://localhost:3001/Admin/sem")
    ...
    const update= (sem_id)=>{
      ...
      Axios.put("http://localhost:3001/Admin/updatesem",{
    ...
    const Delete = (sem_id)=>{
      ...
      Axios.put("http://localhost:3001/Admin/deletesem",{
     ...
     {posts.map((val) => (
        ...
        <TableHead>
          ...
          <IconButton aria-label="expand row" size="small" onClick={() => setOpen(!open)}>
            {open ? <KeyboardArrowUpIcon /> : <KeyboardArrowDownIcon />}
              ...
              <Button variant="contained" color="primary"  onClick={()=>{Delete(val.sem_id)}} >
          Delete
        </Button>
              </StyledTableCell>
      ...
      <TableRow>
       ...
       <TextField
        ...
        <Button variant="contained" color="primary" onClick={()=>{update(val.sem_id)}} >
          Update
        </Button>
